...
class MessageController extends Controller
{
    public function index(){
        ...
        return view('patient.profile.message.index',compact('user','doctors','banner','navigation','text'));
    }
    ...
    public function messageBox($slug){
        ...
        return view('patient.profile.message.single-message',compact('messages','doctors','banner','navigation','text','user','doctor_id'));
    }
    ...
    public function getMessage($doctor_id){
        ...
        Message::where(['user_id'=>$user->id,'doctor_id'=>$doctor_id])->update(['user_view'=>1]);
        $messages = Message::where(['user_id'=>$user->id,'doctor_id'=>$doctor_id])->get();
        return view('patient.profile.message.message-box',compact('messages'));
    }
