...
class AppointmentController extends Controller
{
    ...
    public function getAppointment(Request $request){
        ...
        if($leave ==0){
            ...
            if($schedules->count() !=0){
                ...
                $html="<h4 class='text-danger'>".$text->schedule_not_found."</h4>";
                return response()->json(['error'=>$html]);
            }
        }else{
            $html="<h4 class='text-danger'>.$text->doctor_not_found.</h4>";
            return response()->json(['error'=>$html]);
        }
    }
    ...
    public function getDepartmentDoctor($id){
        ...
        $html='<option value="">'.$text->select_doctor.'</option>';
        if($doctors){
    ...
    public function createAppointment(Request $request){
        ...
        $notification=array(
            'messege'=>$notify->appointment_added,
            'alert-type'=>'success'
        );
    ...
    public function removeAppointment($id){
        ...
        $notify=NotificationText::first();
        $notification=array(
            'messege'=>$notify->appointment_removed,
            'alert-type'=>'success'
        );
